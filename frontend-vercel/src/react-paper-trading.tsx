...
interface AnalyticsData {
    ...
    avg_win?: number;
    avg_loss?: number;
    best_trade?: { realized_pnl: number; symbol: string };
    worst_trade?: { realized_pnl: number; symbol: string };
}
...
const fmt = {
    currency: (n: number | null | undefined, compact = false): string => {
    ...
    shares: (n: number | null | undefined): string => {
        if (n == null || isNaN(n)) return '—';
...
const pnlClass = (n: number | null | undefined): string => (n != null && n > 0 ? 'pt-positive' : n != null && n < 0 ? 'pt-negative' : 'pt-neutral');
...
const PortfolioChart = ({ history, initialBalance }: { history: HistoryEntry[]; initialBalance: number }) => {
        ...
        chartRef.current = new ChartJS(ctx, {
            ...
            options: {
                ...
                plugins: {
                    ...
                    tooltip: {
                        callbacks: {
                            label: (ctx: any) => fmt.currency(ctx.parsed.y),
                        },
...
const TradeForm = ({ onOrderPlaced, portfolioSummary }: { onOrderPlaced: () => void; portfolioSummary: Portfolio | null }) => {
    ...
    // Search suggestions
    const searchSymbol = useCallback(async (query: string) => {
    ...
    useEffect(() => {
        const handler = (e: Event) => {
            const { symbol: sym, side: s, quantity: q } = (e as CustomEvent).detail || {};
...
const PositionsTable = ({ positions }: { positions: Position[] }) => {
    ...
    const handleSellAll = (pos: Position) => {
...
const OrdersTable = ({ orders, onCancel }: { orders: Order[]; onCancel: (id: string) => void }) => {
...
const AnalyticsPanel = ({ analytics }: { analytics: AnalyticsData | null }) => {
    if (!analytics) return null;

    const stats: { label: string; value: string | number; icon: string; className?: string }[] = [
...
const PaperTradingView = () => {
    ...
    // Stat cards
    const statCards: { label: string; value: string; icon: string; sub: string | null; className?: string }[] = [
        {
