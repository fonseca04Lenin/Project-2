...
const LoadingState = ({ messages }: { messages: string[] }) => {
    ...
    return (
        <div style={{ display: 'flex', flexDirection: 'column', gap: '0.7rem', padding: '0.25rem 0' }}>
            <div style={{ display: 'flex', alignItems: 'center', gap: '0.6rem', marginBottom: '0.1rem' }}>
                <div style={{ width: 7, height: 7, borderRadius: '50%', background: '#00D924', animation: 'dot-pulse 1.4s ease-in-out infinite', flexShrink: 0 }} />
                <span style={{ color: 'rgba(255,255,255,0.42)', fontSize: '0.82rem' }}>{messages[msgIdx]}</span>
            </div>
            <SkeletonBlock height="0.9rem" width="55%" />
            <SkeletonBlock height="0.9rem" />
            <SkeletonBlock height="0.9rem" width="90%" />
            <SkeletonBlock height="0.9rem" width="72%" />
        </div>
    );
};

const UpgradeGate = ({ message, usedDaily = false }: { message: string; usedDaily?: boolean }) => (
    <div style={{
        textAlign: 'center',
        padding: '3rem 2rem',
        ...
        <div style={{
            ...
            background: usedDaily ? 'linear-gradient(135deg, #374151, #6B7280)' : 'linear-gradient(135deg, #FFB800, #FF6B35)',
            display: 'flex', alignItems: 'center', justifyContent: 'center',
            margin: '0 auto 16px', fontSize: 22,
        }}>
            <i className={`fas ${usedDaily ? 'fa-clock' : 'fa-crown'}`}></i>
        </div>
        <div style={{ color: '#fff', fontWeight: 700, fontSize: '1rem', marginBottom: 8 }}>
            {usedDaily ? 'Daily limit reached' : 'Pro Feature'}
        </div>
        <div style={{ color: 'rgba(255,255,255,0.5)', fontSize: '0.87rem', maxWidth: 360, margin: '0 auto 20px', lineHeight: 1.6 }}>
            {usedDaily
                ? "You've used your 1 free access for today. Come back tomorrow or upgrade for unlimited use."
                : message}
        </div>
        <button
        ...
        {usedDaily && (
            <p style={{ margin: '12px 0 0', color: 'rgba(255,255,255,0.22)', fontSize: '0.75rem' }}>
...
// ── Tab components at module level so their identity is stable ──

const MorningBriefTab = ({ watchlistData }: { watchlistData: StockItem[] }) => {
        ...
        try {
            ...
            if (r.status === 403) {
                ...
                if (j.error === 'upgrade_required') {
                    setError(j.message?.includes('used your free') ? 'daily_limit_hit' : 'upgrade_required');
                    return;
                }
            }
            if (!r.ok) {
    ...
    return (
        <div style={{ maxWidth: '820px' }}>
            ...
            {(error === 'upgrade_required' || error === 'daily_limit_hit') ? (
                <UpgradeGate message="Morning Brief gives you a personalized daily AI briefing on your watchlist's top movers and upcoming earnings." usedDaily={error === 'daily_limit_hit'} />
            ) : error && (
            ...
            <div className="intel-card" style={{ marginBottom: '1rem' }}>
                {loading ? (
                    <LoadingState messages={['Scanning your watchlist...', 'Pulling price data...', 'Writing your brief...']} />
                ) : briefData ? (
...
const ThesisBuilderTab = ({ watchlistData }: { watchlistData: StockItem[] }) => {
    ...
    return (
        <div style={{ maxWidth: '960px' }}>
            ...
            <div style={{ display: 'flex', gap: '0.75rem', alignItems: 'center', marginBottom: '1.5rem' }}>
                ...
                {error && error !== 'upgrade_required' && error !== 'daily_limit_hit' && <span style={{ color: '#FF6B35', fontSize: '0.85rem' }}>{error}</span>}
            </div>

            {(error === 'upgrade_required' || error === 'daily_limit_hit') && (
                <UpgradeGate message="Thesis Builder generates AI-powered bull and bear investment cases grounded in live market data." usedDaily={error === 'daily_limit_hit'} />
            )}

            {loading && (
                <div className="intel-card">
                    <LoadingState messages={['Pulling market data...', 'Running valuation models...', 'Building the thesis...']} />
                </div>
            )}
...
const HealthScoreTab = ({ watchlistData }: { watchlistData: StockItem[] }) => {
    const [loading, setLoading] = useState(false);
    const [data, setData] = useState<Record<string, unknown> | null>(null);
    const [error, setError] = useState('');
    ...
    return (
        <div style={{ maxWidth: '860px' }}>
            ...
            {(error === 'upgrade_required' || error === 'daily_limit_hit') ? (
                <UpgradeGate message="Portfolio Health Score gives your watchlist an AI-powered grade with diversification analysis and actionable suggestions." usedDaily={error === 'daily_limit_hit'} />
            ) : error && (
                <div style={{ color: '#FF6B35', marginBottom: '1rem', padding: '0.75rem 1rem', background: 'rgba(255,107,53,0.1)', borderRadius: '8px' }}>{error}</div>
            )}

            {error !== 'upgrade_required' && error !== 'daily_limit_hit' && loading && (
                <div className="intel-card">
                    <LoadingState messages={['Analyzing your holdings...', 'Running diversification metrics...', 'Calculating your grade...']} />
                </div>
            )}
...
const SectorRotationTab = ({ watchlistData }: { watchlistData: StockItem[] }) => {
    const [loading, setLoading] = useState(false);
    const [data, setData] = useState<Record<string, unknown> | null>(null);
    const [error, setError] = useState('');
    ...
    return (
        <div style={{ maxWidth: '920px' }}>
            ...
            {(error === 'upgrade_required' || error === 'daily_limit_hit') ? (
                <UpgradeGate message="Sector Rotation tracks institutional money flow across all 11 S&P sectors with AI-generated strategy notes." usedDaily={error === 'daily_limit_hit'} />
            ) : error && (
                <div style={{ color: '#FF6B35', marginBottom: '1rem', padding: '0.75rem 1rem', background: 'rgba(255,107,53,0.1)', borderRadius: '8px' }}>{error}</div>
            )}

            {error !== 'upgrade_required' && error !== 'daily_limit_hit' && (<div className="intel-card" style={{ marginBottom: '1.25rem', minHeight: '80px' }}>
                {loading ? (
                    <LoadingState messages={['Fetching sector performance...', 'Reading the money flows...', 'Writing market notes...']} />
                ) : (
                    <p style={{ margin: 0, lineHeight: 1.7, color: 'rgba(255,255,255,0.85)', fontSize: '0.95rem' }}>
                        {displayedText}
...
const EarningsBreakdownTab = ({ watchlistData }: { watchlistData: StockItem[] }) => {
    const [symbol, setSymbol] = useState('');
    const [loading, setLoading] = useState(false);
    const [data, setData] = useState<Record<string, unknown> | null>(null);
    const [error, setError] = useState('');
    ...
    return (
        <div style={{ maxWidth: '860px' }}>
            ...
            {(error === 'upgrade_required' || error === 'daily_limit_hit') && (
                <UpgradeGate message="Earnings Breakdown gives you an AI-written analysis of any stock's recent earnings — what happened, what it means, and what to watch next." usedDaily={error === 'daily_limit_hit'} />
            )}

            {loading && (
                <div className="intel-card">
                    <LoadingState messages={['Pulling earnings data...', 'Reading the financials...', 'Writing the breakdown...']} />
                </div>
            )}
...
const PortfolioGuidanceTab = ({ watchlistData }: { watchlistData: StockItem[] }) => {
    const [loading, setLoading] = useState(false);
    const [data, setData] = useState<Record<string, unknown> | null>(null);
    const [error, setError] = useState('');
    ...
    return (
        <div style={{ maxWidth: '820px' }}>
            ...
            {(error === 'upgrade_required' || error === 'daily_limit_hit') ? (
                <UpgradeGate message="Portfolio Guidance gives you an AI advisor's frank read on your holdings — sector exposure, valuation risk, and specific actionable steps." usedDaily={error === 'daily_limit_hit'} />
            ) : error && (
                <div style={{ color: '#FF6B35', marginBottom: '1rem', padding: '0.75rem 1rem', background: 'rgba(255,107,53,0.1)', borderRadius: '8px' }}>{error}</div>
            )}
            ...
            {error !== 'upgrade_required' && error !== 'daily_limit_hit' && loading && (
                <div className="intel-card">
                    <LoadingState messages={['Reviewing your positions...', 'Checking sector exposure...', 'Preparing guidance...']} />
                </div>
            )}
...
const AISuiteView = ({ watchlistData, defaultTab = 'brief', onTabChange }: { watchlistData: StockItem[]; defaultTab?: string; onTabChange?: (tab: string) => void }) => {
    ...
    const visitedRef = useRef<Set<string>>(new Set([defaultTab]));

    useEffect(() => {
        setActiveTab(defaultTab);
        visitedRef.current.add(defaultTab);
    }, [defaultTab]);
    ...
    return (
        <div className="intelligence-view">
            ...
            <div style={{ padding: '0.5rem 0' }}>
                {visitedRef.current.has('brief') && (
                    <div style={{ display: activeTab === 'brief' ? '' : 'none' }}>
                        <MorningBriefTab watchlistData={watchlistData} />
                    </div>
                )}
                {visitedRef.current.has('thesis') && (
                    <div style={{ display: activeTab === 'thesis' ? '' : 'none' }}>
                        <ThesisBuilderTab watchlistData={watchlistData} />
                    </div>
                )}
                {visitedRef.current.has('health') && (
                    <div style={{ display: activeTab === 'health' ? '' : 'none' }}>
                        <HealthScoreTab watchlistData={watchlistData} />
                    </div>
                )}
                {visitedRef.current.has('rotation') && (
                    <div style={{ display: activeTab === 'rotation' ? '' : 'none' }}>
                        <SectorRotationTab watchlistData={watchlistData} />
                    </div>
                )}
                {visitedRef.current.has('earnings') && (
                    <div style={{ display: activeTab === 'earnings' ? '' : 'none' }}>
                        <EarningsBreakdownTab watchlistData={watchlistData} />
                    </div>
                )}
                {visitedRef.current.has('portfolio') && (
                    <div style={{ display: activeTab === 'portfolio' ? '' : 'none' }}>
                        <PortfolioGuidanceTab watchlistData={watchlistData} />
                    </div>
                )}
            </div>
        </div>
