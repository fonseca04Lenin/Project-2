...
Co
# =============================================================================
# COMPANY LOCATIONS API - For Map Feature
# =============================================================================
...
@app.route('/api/companies/locations')
    ...
    try:
        ...
        def fetch_company_info(symbol):
            try:
                ...
                # Skip non-US companies or those without location
                if country and country != 'United States':
                    return None
                if not city:
                    return None

                # Get coordinates using a simple US city lookup or geocoding
                # For now, we'll include the city/state and let frontend handle geocoding if needed
                ...
                return {
                    ...
                    # Include raw city/state for geocoding
                    'rawCity': city,
                    'rawState': state
                }
            except Exception as e:
        ...
        # Use ThreadPoolExecutor for parallel fetching (faster)
        # Limit to 50 companies at a time to avoid rate limiting
        symbols_to_fetch = MAJOR_COMPANY_SYMBOLS[:100]  # Start with top 100
...
def youtube_search():
    ...
    query = request.args.get('q', '').strip()
    max_results = min(int(request.args.get('max_results', 5)), 10)  # Cap at 10
